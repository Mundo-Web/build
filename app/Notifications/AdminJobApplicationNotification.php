...
class AdminJobApplicationNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {
        Log::info('Enviando notificación de solicitud de trabajo al administrador: ' . $notifiable->description);
        
        $template = \App\Models\General::where('correlative', 'admin_job_application_email')->first();
        $body = $template
        ...
        return (new RawHtmlMail($body, 'Nueva solicitud de empleo: ' . $this->jobApplication->name, $notifiable->email));
    }
}