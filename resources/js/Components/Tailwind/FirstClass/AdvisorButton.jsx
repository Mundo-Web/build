...
/**
 * Componente reutilizable para botones de asesor/contacto con WhatsApp
 * Similar a WhatsApp.jsx pero solo maneja funcionalidad, acepta children para diseño personalizado
 * Soporta un solo asesor (abre directo) o múltiples asesores (muestra dropdown)
 * 
 * @param {Object} props
 ...
 */
const AdvisorButton = ({ className = "", children, onClick }) => {
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const dropdownRef = useRef(null);
    
    // Obtener asesores de WhatsApp desde General
    ...
    const handleClick = (e) => {
        // Si no hay asesores, no hacer nada
        ...
        if (advisors.length === 1) {
            ...
            const message = advisor.message || '¡Hola! Necesito información sobre sus servicios';
            window.open(
    ...
    const handleAdvisorClick = (advisor) => {
        const message = advisor.message || '¡Hola! Necesito información';
        window.open(
            `https://api.whatsapp.com/send?phone=${advisor.phone}&text=${encodeURIComponent(message)}`, 
            '_blank'
        );
    ...
    // Si no hay asesores, no mostrar el botón
    ...
    return (
        <div ref={dropdownRef} className="relative inline-block">
            <button
                onClick={handleClick}
                className={className}
                aria-label="Hablar con un asesor especializado"
            >
                {children}
            </button>
            ...
            <AnimatePresence>
                {isDropdownOpen && advisors.length > 1 && (
                    <motion.div
                        ...
                        className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-white rounded-2xl shadow-2xl overflow-hidden z-50"
