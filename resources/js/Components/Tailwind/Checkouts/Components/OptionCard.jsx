...
const OptionCard = ({ 
    ...
    const handleConsultClick = (e) => {
        ...
        if (advisors.length === 1) {
            ...
            window.open(
                `https://api.whatsapp.com/send?phone=${advisor.phone}&text=${encodeURIComponent(consultMessage || '¡Hola! Necesito información sobre el costo de envío')}`,
                '_blank'
            );
        } else {
    ...
    return (
        <div
            className={`flex flex-col w-full gap-3 p-4 border-2 rounded-xl transition-all duration-300 customtext-neutral-dark ${
        ...
        >
            ...
            {/* Botón de consultar costo de envío por WhatsApp */}
            {showConsultButton && advisors.length > 0 && (
                <div className="relative" ref={dropdownRef}>
                    <button
                        type="button"
                        onClick={handleConsultClick}
                        className={`w-full mt-1 px-4 py-2.5 bg-green-500 hover:bg-green-600 text-white text-sm font-medium transition-colors duration-200 flex items-center justify-center gap-2 ${class_advisors}`}
                    >
                    ...
                    {showAdvisorsDropdown && advisors.length > 1 && (
                        <div 
                            className="absolute bottom-full left-0 right-0 mb-2 bg-white rounded-xl shadow-2xl overflow-hidden z-50 border border-gray-200"
                            onClick={(e) => e.stopPropagation()}
                        >
                            {/* Header */}
                            <div className="bg-gradient-to-r from-green-500 to-green-600 p-3 text-white">
                                <h3 className="font-bold text-sm">Elige un asesor</h3>
                                <p className="text-xs text-green-100">¿Con quién quieres hablar?</p>
                            </div>
