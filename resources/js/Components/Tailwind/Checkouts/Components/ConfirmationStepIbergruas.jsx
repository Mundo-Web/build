...
export default function ConfirmationStepIbergruas({ 
    setCart, 
    cart, 
    code, 
    delivery, 
    ...
    if (loading) {
        return (
            <div className="mx-auto">
                <div className="bg-transparent rounded-lg shadow p-6 font-paragraph text-center">
    ...
    if (error) {
        return (
            <div className="mx-auto">
                <div className="bg-transparent rounded-lg shadow p-6 font-paragraph text-center text-red-500">
    ...
    return (
        <div className="mx-auto">
            <div className="bg-transparent rounded-lg shadow p-6 font-paragraph">
                <div className="text-center space-y-2">
                    <h2 className="text-base xl:text-xl customtext-neutral-light">Gracias por tu compra 🎉</h2>
                    <p className="text-white text-2xl xl:text-5xl font-semibold">Tu orden ha sido recibida</p>

                    <div className="py-4">
                        <div className=" customtext-neutral-light">Código de pedido</div>
                        <div className="text-white text-lg font-semibold">#{order.code}</div>
                    </div>

                    <div className="space-y-4 text-white max-w-lg bg-transparent mx-auto p-8 rounded-xl">
                        <div className="space-y-6 border-b-2 pb-6">
                            {order.items.map((item, index) => (
                                <div key={index} className="rounded-lg">
                                    <div className="flex gap-4">
                                        <div className="bg-transparent rounded-xl w-max">
                                            <img
                                        ...
                                        <div className="text-start">
                                            ...
                                            <p className="text-sm customtext-neutral-light">
                                                Cantidad: <span className="text-white">{parseInt(item.quantity)}</span> -
                                                Precio: <span className="text-white"> {CurrencySymbol()} {Number2Currency(item.price)}</span>
                                                {item.is_free && (
                                    ...
                                    <div className="pt-4 border-t border-dashed border-green-300">
                                        ...
                                        {order.free_items.map((item, index) => (
                                            <div key={`free-${index}`} className="rounded-lg">
                                                <div className="flex gap-4">
                                                    <div className="bg-transparent rounded-xl w-max border-2 border-green-200">
                                                        <img
                                                            src={item.image ? `/storage/images/item/${item.image}` : '/assets/img/noimage/no_img.jpg'}
                                                            alt={item.name}
                                                            className="w-20 h-20 object-cover rounded"
                        ...
                        <div className="pt-3">
                            <ButtonPrimary href="/catalogo" className={` !rounded-none customtext-neutral-dark ${data?.class_button} `}>  Seguir Comprando</ButtonPrimary>
