...
const CartSimple = ({ data, cart, setCart }) => {
    const totalPrice = cart.reduce((acc, item) => {
        ...
        if (item.type === "booking") {
            ...
            finalPrice =
                item.discount > 0
                    ? Math.min(item.price, item.discount)
                    : item.price;
        } else {
                        ...
                        <div className="grid items-center justify-center h-full">
                            <div>
                                ...
                                <button
                                    ...
                                    className="bg-primary p-2 px-4 rounded-full text-white block mx-auto"
                                >
                    ...
                    {hasBookings && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 my-3 text-sm">
                            ...
                            <span className="text-blue-800">
                                Tu carrito incluye{" "}
                                {hasProducts ? "productos y " : ""}reserva
                                {cart.filter((x) => x.type === "booking")
                                    .length > 1
                                    ? "s"
                                    : ""}{" "}
                                de hotel.
                    ...
                    <div>
                        <div className="flex flex-col gap-5">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span>
                                    {CurrencySymbol()}{" "}
                                    {Number2Currency(totalPrice - subTotal)}
                                </span>
                            </div>
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-semibold text-[20px]">
                                    {CurrencySymbol()}{" "}
                                    {Number2Currency(totalPrice)}
                                </span>
                            </div>
                            <button
                                href={data?.url_checkout}
                                className="text-white bg-primary w-full px-4 py-2 rounded-full cursor-pointer inline-block text-center"
                            >
                                Siguiente
                            </button>
                        </div>
