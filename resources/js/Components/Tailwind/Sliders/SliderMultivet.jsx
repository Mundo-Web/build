...
const SliderMultivet = ({ items, data, generals = [] }) => {
        ...
        return parts.map((part, index) => {
            if (part.startsWith("*") && part.endsWith("*")) {
                ...
                return (
                    <span
                        key={index}
                        className="font-title customtext-accent !uppercase font-extrabold"
                    >
                        {highlightedText}
                    </span>
                );
    ...
    return (
        <section
            ...
            className="relative h-[calc(100dvh-12dvh)] lg:h-[700px] overflow-hidden"
        >
            {/* Slides Container */}
            <div
            ...
            {showPagination && sortedItems.length > 1 && (
                <div className="absolute bottom-4 lg:bottom-8 left-1/2 transform -translate-x-1/2 z-20">
                    <div className="flex items-center space-x-6 bg-black/30 backdrop-blur-sm rounded-full px-6 py-3 border-none">
                        {/* Dots Indicator */}
