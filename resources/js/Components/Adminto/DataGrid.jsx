...
const DataGrid = ({
        ...
        if (result?.limits?.length) {
            ...
            setBooleanLimitForm(
                result.limits.map(({ field, limit }) => ({
                    ...
                    general_key:
                        limit.general_key ??
                        payload.find((item) => item.field === field)
                            ?.general_key,
                    model: currentModel,
                })),
            );
    ...
    const onModalSubmit = async (e) => {
        ...
        const checkboxes = document.querySelectorAll(
            `input[type="checkbox"][id^="fillable-ck-${currentModel}"]`,
        );
        ...
        $(dataGridRef.current)
            .dxDataGrid({
                ...
                onToolbarPreparing: (e) => {
                    ...
                    isAdmin &&
                        currentModel &&
                        BooleanLimit.list(currentModel).length > 0 &&
                        items.unshift({
                            widget: "dxButton",
                            location: "before",
                            options: {
        ...
        <>
            ...
            >
                <ul class="nav nav-tabs nav-bordered">
                    {Fillable.models.map((model, index) => (
                        <li key={index} class="nav-item">
                            <button
                                href={`#fillable-${model}`}
                                data-bs-toggle="tab"
                                aria-expanded="false"
                                class={`nav-link ${activeModel == model && "active"}`}
                ...
                <div class="tab-content" style={{ minHeight: "240px" }}>
                    {Fillable.models.map((model, index) => (
                        <div
                            key={index}
                            class={`tab-pane ${activeModel == model && "active"}`}
                            id={`fillable-${model}`}
                        >
                            <div
                                ...
                                style={{ gap: "6px" }}
                            >
                                {Object.keys(Fillable.values[model]).map(
                                    (key, index) => {
                                        return (
                                            <label
                                                key={index}
                                                class="form-check form-check-success border cursor-pointer"
                                                htmlFor={`fillable-ck-${model}-${key}`}
                                                style={{
                                                    padding:
                                                        "2px 10px 2px 30px",
                                                    borderRadius: "8px",
                                            ...
                                            >
                                                <input
                                                    ...
                                                    type="checkbox"
                                                    value={key}
                                                    id={`fillable-ck-${model}-${key}`}
                                                    defaultChecked={
                                                        Fillable.values[model][
                                                            key
                                                        ]
                                                    }
                                                />
            ...
            <Modal
                modalRef={booleanLimitModalRef}
                title="Configurar límites"
                onSubmit={onBooleanLimitSubmit}
                loading={savingBooleanLimits}
                onClose={() => setBooleanLimitForm([])}
            >
                {!booleanLimitForm.length && (
                        ...
                        <div className="col-md-3">
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    onBooleanLimitFieldChange(index, {
