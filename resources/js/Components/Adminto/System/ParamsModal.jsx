...
const ParamsModal = ({ dataLoaded, setDataLoaded, setPages, modalRef, models }) => {
  ...
  useEffect(() => {
    ...
    const nonRouteKeys = keysInUsing.filter(key => !routeParams.includes(key));
    setExtraKeys(nonRouteKeys);
  }, [dataLoaded]);

  const addExtraKey = () => {
    const newKey = prompt('Ingrese el nombre de la clave (ej: categories, brands):');
    if (newKey && !routeParams.includes(newKey) && !extraKeys.includes(newKey)) {
  ...
  return (
    <Modal modalRef={modalRef} title={`Gestionar datos de la página - ${dataLoaded?.name}`} onSubmit={onParamsSubmit} onClose={() => setDataLoaded(null)}>
      <div className="d-flex flex-column gap-3">
        ...
        <div className="text-center mt-2">
          <button type="button" className="btn btn-sm btn-outline-primary" onClick={addExtraKey}>
