...
const BasicEditing4System = (page) => {
    ...
    const [referrableEditing, setReferrableEditing] = useState(false);
    ...
    const onNameChange = async (e) => {
        ...
        setPages((old) =>
            old.map((page) =>
                page.id === id ? { ...page, name: e.target.value } : page,
            ),
        );
    };
    ...
    const onPathChange = async (e) => {
        ...
        setPages((old) =>
            old.map((page) =>
                page.id === id ? { ...page, path: e.target.value } : page,
            ),
        );
    };
    ...
    const onExtendsBaseChange = async (e) => {
        ...
        setPages((old) =>
            old.map((page) =>
                page.id === id ? { ...page, extends_base: checked } : page,
            ),
        );
    };
    ...
    const onMenuableChange = async (e) => {
        ...
        setPages((old) =>
            old.map((page) =>
                page.id === id ? { ...page, menuable: checked } : page,
            ),
        );
    };
    ...
    const onSitemapableChange = async (e) => {
        ...
        setPages((old) =>
            old.map((page) =>
                page.id === id ? { ...page, sitemapable: checked } : page,
            ),
        );
    };

    const onReferrableChange = async (e) => {
        setReferrableEditing(true);
        const checked = e.target.checked;
        const result = systemRest.savePage({
            id,
            referrable: checked,
        });
        setReferrableEditing(false);
        if (!result) return;
        setPages((old) =>
            old.map((page) =>
                page.id === id ? { ...page, referrable: checked } : page,
            ),
        );
    };
        ...
        <>
            <div className="dropdown float-end">
                ...
                <div className="dropdown-menu dropdown-menu-end">
                    ...
                    <button
                        className="dropdown-item"
                        onClick={() => onDeletePageClicked(page)}
                    >
            ...
            <div>
                {nameEditing ? (
                    ...
                    <Tippy content="Editar nombre" placement="left">
                        <h4
                            className="header-title mb-1 d-block"
            ...
            <div className="row">
                <SwitchFormGroup
                    label="Extiende de base"
                    col={"col-lg-6 col-md-12 col-sm-6"}
                    checked={extends_base}
                    onChange={onExtendsBaseChange}
                    disabled={extendsBaseEditing}
                />
                <SwitchFormGroup
                    label="Mostrar en menu"
                    col={"col-lg-6 col-md-12 col-sm-6"}
                    checked={menuable}
                    onChange={onMenuableChange}
                    disabled={menuableEditing}
                />
                <SwitchFormGroup
                    label="Incluir en sitemap"
                    col={"col-lg-6 col-md-12 col-sm-6"}
                    checked={sitemapable}
                    onChange={onSitemapableChange}
                    disabled={sitemapableEditing}
                />
                <SwitchFormGroup
                    label="Procesar referidos"
                    col={"col-lg-6 col-md-12 col-sm-6"}
                    checked={referrable}
                    onChange={onReferrableChange}
                    disabled={referrableEditing}
                />
            </div>
        </>
    );
