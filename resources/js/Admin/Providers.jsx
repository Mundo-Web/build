...
const Providers = ({ }) => {
  ...
  const onStatusChange = async ({ id, value }) => {
    ...
    const result = await providersRest.boolean({ id, field: 'status', value })
    if (!result) return
    $(gridRef.current).dxDataGrid('instance').refresh()
  }
  ...
  return (<>
    <Table gridRef={gridRef} title='Proveedores' rest={providersRest}
      toolBar={(container) => {
        container.unshift({
      ...
      columns={[
            ...
            ReactAppend(container, 
              <div>
                ...
                <small className="text-muted">{data.email}</small>
                 <br />
                <span className="badge badge-soft-primary" style={{fontSize: '10px'}}>{data.uuid}</span>
              </div>
            )
          }
        },
        ...
        },
        {
          ...
          cellTemplate: (container, { data }) => {
            if (data.status == 1) {
               ReactAppend(container, <span className='badge badge-success'>Activo</span>)
            } else {
               ReactAppend(container, <span className='badge badge-secondary'>Archivado</span>)
            }
          }
        },
        {
          ...
          cellTemplate: (container, { data }) => {
            ...
            if (data.status != 1) {
               container.append(DxButton({
                ...
                icon: 'fa fa-check',
                onClick: () => onStatusChange({ id: data.id, value: true }) // Reactivar
              })) 
            }
            ...
            container.append(DxButton({
              ...
              title: data.status == 1 ? 'Archivar' : 'Eliminar permanentemente',
              icon: data.status == 1 ? 'fa fa-archive' : 'fa fa-trash',
              onClick: async () => {
                 if (data.status == 1) {
                   // Lógica de Archivar
                   const { isConfirmed } = await Swal.fire({
                    ...
                    if (isConfirmed) onStatusChange({ id: data.id, value: false })
                 } else {
    ...
    <Modal modalRef={modalRef} title={isEditing ? 'Editar proveedor' : 'Crear nuevo proveedor'} onSubmit={onModalSubmit} size='lg'>
      <div className='row' id='principal-container'>
        ...
        <InputFormGroup eRef={dniRef} label='DNI / Documento' col='col-md-6' />
        
        <div className="col-md-3">
          <SelectFormGroup eRef={phonePrefixRef} label='Prefijo' required dropdownParent='#principal-container'>
