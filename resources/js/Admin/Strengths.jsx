...
const Strengths = ({ details }) => {
  ...
  const onModalOpen = (data) => {
    ...
    if (Fillable.has('strengths', 'name')) {
      nameRef.current.value = data?.name ?? ''
    }
    if (Fillable.has('strengths', 'description')) {
      descriptionRef.current.value = data?.description ?? ''
    }

    // Configurar color de fondo
    if (Fillable.has('strengths', 'bg_color')) {
    ...
    if (Fillable.has('strengths', 'image') && imageRef.current && imageRef.image) {
  ...
  const onModalSubmit = async (e) => {
    ...
    if (imageRef.current && Fillable.has('strengths', 'image')) {
  ...
  const onReorder = async (e) => {
    ...
    try {
      const result = await strengthsRest.reorder(e.itemData.id, newOrderIndex)
      if (result) {
        await e.component.refresh()
      }
    } catch (error) {
      console.error('Error reordering strength:', error)
    }
  }
  ...
  return (<>
    <Table gridRef={gridRef} title={<BasicEditing correlative='values' details={details} />} rest={strengthsRest}
      ...
      columns={[
        ...
        Fillable.has('strengths', 'name') && {
          dataField: 'name',
          caption: 'Fortaleza',
          width: '30%',
        },
        Fillable.has('strengths', 'description') && {
          dataField: 'description',
          caption: 'Descripción',
          width: '50%',
        },
        Fillable.has('strengths', 'image') && {
          dataField: 'image',
          caption: 'Imagen',
          cellTemplate: (container, { data }) => {
            const imageUrl = data.image ? `/storage/images/strength/${data.image}` : '/api/cover/thumbnail/null'
            const bgColor = data.bg_color || 'transparent'
        ...
        Fillable.has('strengths', 'visible') && {
          dataField: 'visible',
          caption: 'Visible',
          dataType: 'boolean',
          cellTemplate: (container, { data }) => {
    ...
    <Modal modalRef={modalRef} title={isEditing ? 'Editar fortaleza' : 'Agregar fortaleza'} onSubmit={onModalSubmit} size='lg'>
          ...
          {Fillable.has('strengths', 'image') && (
            <div className="tab-pane fade" id="multimedia" role="tabpanel" aria-labelledby="multimedia-tab">
              <div className="row g-3">
                ...
                {Fillable.has('strengths', 'bg_color') && (
                  <div className="col-6">
                    <div className="card border-0 shadow-sm">
                      <div className="card-header bg-light">
                        <h6 className="mb-0">
