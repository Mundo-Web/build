...
const Sales = ({ statuses = [], hasRootRole = false }) => {
    ...
    const exportableFieldsCategories = {
        ...
        "Totales y Costos de la Venta": {
            ...
            VENTA_COSTO_EMPAQUE: "Costo de Empaque",
    ...
    const exportToExcel = async (filters = {}) => {
        try {
            ...
            const calculateProportionalPrices = (
                comboItems,
                comboFinalPrice,
                comboOriginalPrice,
            ) => {
                ...
                if (totalOriginalPrice === 0) {
                    ...
                    return comboItems.map((item) => ({
                        ...item,
                        proportional_price:
                            pricePerItem / (parseInt(item.quantity) || 1),
                    }));
                }
            ...
            const filterFieldsByConfig = (rowData) => {
                ...
                const fieldMapping = {
                    ...
                    VENTA_COSTO_EMPAQUE: "VENTA_COSTO_EMPAQUE",
            ...
            salesData.forEach((sale) => {
                ...
                if (sale.details && sale.details.length > 0) {
                    sale.details.forEach((product, productIndex) => {
                        ...
                        if (
                            (product.type === "combo" || product.combo_id) &&
                            product.combo_data &&
                            product.combo_data.items &&
                            product.combo_data.items.length > 0
                        ) {
                            ...
                            const comboItems = calculateProportionalPrices(
                                ...
                                product.combo_data.original_price ||
                                    product.price,
                            );
                            ...
                            comboItems.forEach((comboItem, comboItemIndex) => {
                                globalProductIndex++;
                                const itemQuantity =
                                    (parseInt(comboItem.quantity) || 1) *
                                    parseInt(product.quantity);
                                const itemUnitPrice =
                                    parseFloat(comboItem.proportional_price) ||
                                    0;
                                const itemSubtotal =
                                ...
                                const rowData = {
                                    ...
                                    VENTA_COSTO_EMPAQUE: sale.packaging_amount,
                                    VENTA_SEGURO_IMPORTACION:
                                        sale.seguro_importacion_total || 0,
                                    VENTA_DERECHO_ARANCELARIO:
                                        sale.derecho_arancelario_total || 0,
                                    VENTA_FLETE_TOTAL: sale.flete_total || 0,
                                    VENTA_DESCUENTO_PAQUETE:
                                        sale.bundle_discount,
                                    VENTA_DESCUENTO_RENOVACION:
                                        sale.renewal_discount,
                                    VENTA_DESCUENTO_CUPON: sale.coupon_discount,
                                    VENTA_CODIGO_CUPON: sale.coupon_code,
                                    VENTA_DESCUENTO_PROMOCION:
                                        sale.promotion_discount,
                                    VENTA_PROMOCIONES_APLICADAS:
                                        sale.applied_promotions,
                                    VENTA_TOTAL_FINAL: sale.total_amount,

                                    // INDICADORES DE FILA
                                    PRODUCTO_NUMERO: globalProductIndex,
                                    TOTAL_PRODUCTOS_EN_VENTA:
                                        sale.details.length,
                                    ES_PRIMER_PRODUCTO:
                                        globalProductIndex === 1 ? "SI" : "NO",
                                    ES_COMBO_ITEM: "SI",
            ...
            const hasEnabledFields = Object.values(configToUse).some(
                (value) => value === true,
            );
            ...
            const columnWidths = enabledColumns.map(
                (column) => allColumnWidths[column] || { wch: 15 },
            );
            ...
            for (let row = 1; row <= range.e.r; row++) {
                for (let col = range.s.c; col <= range.e.c; col++) {
                    ...
                    const esComboItemCol = Object.keys(
                        excelData[0] || {},
                    ).indexOf("ES_COMBO_ITEM");
                    const isComboItem =
                        excelData[row - 1] &&
                        excelData[row - 1]["ES_COMBO_ITEM"] === "SI";
                    ...
                    worksheet[cellAddress].s = {
                        ...
                        alignment: {
                            horizontal:
                                col >= 23 && col <= 25 ? "right" : "left", // Columnas de precios alineadas a la derecha
                            vertical: "center",
                        },
                        border: {
            ...
            if (filters.status) {
                const statusName =
                    saleStatuses.find((s) => s.id == filters.status)?.name ||
                    "estado";
            ...
            if (filters.status) {
                const statusName = saleStatuses.find(
                    (s) => s.id == filters.status,
                )?.name;
                filterInfo += `\n📊 Estado: ${statusName}`;
            }
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "No se pudo exportar los datos. Intente nuevamente.",
                icon: "error",
            });
        }
    };
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <span className="badge bg-purple-soft text-purple">
                                                        ...
                                                        {saleLoaded.referrer
                                                            .fullname ||
                                                            `${saleLoaded.referrer.name} ${saleLoaded.referrer.lastname || ""}`}
                                                    </span>
                                                </td>
                                            </tr>
                                        )}
                                                ...
                                                <td>
                                                    {saleLoaded?.applied_promotions &&
                                                        (() => {
                                                            try {
                                                                const promotions =
                                                                    typeof saleLoaded.applied_promotions ===
                                                                    "string"
                                                                        ? JSON.parse(
                                                                              saleLoaded.applied_promotions,
                                                                          )
                                                                ...
                                                                console.error(
                                                                    "Error parsing applied_promotions:",
                                                                    e,
                                                                );
                                                                return null;
                                                            }
                        ...
                        <div className="card">
                            ...
                            <div className="card-body p-2">
                                ...
                                {saleLoaded?.perception_amount > 0 && (
                                    <div className="d-flex justify-content-between">
                                        <b>
                                            Percepción ({Global.PERCEPTION_RATE}
                                            %):
                                        </b>
                                        <span>
                                            {CurrencySymbol()}{" "}
                                            {Number2Currency(
                                                saleLoaded?.perception_amount,
                                            )}
                                        </span>
                                    </div>
                                )}
                                ...
                                {saleLoaded?.additional_shipping_cost > 0 && (
                                    <div className="d-flex justify-content-between text-warning">
                                        ...
                                        <span>
                                            {CurrencySymbol()}{" "}
                                            {Number2Currency(
                                                saleLoaded?.additional_shipping_cost,
                                            )}
                                        </span>
                                    </div>
                                )}
                                ...
                                {(saleLoaded?.seguro_importacion_total > 0 ||
                                    ...
                                    <>
                                        {saleLoaded?.seguro_importacion_total >
                                            0 && (
                                        ...
                                        {saleLoaded?.derecho_arancelario_total >
                                            0 && (
                                            <div className="d-flex justify-content-between text-info">
                                ...
                                <small className="text-muted mt-2 d-block">
                                    ...
                                    (envio)
                                    {saleLoaded?.igv_amount > 0 &&
                                        ` (incluye ${Number2Currency(saleLoaded?.igv_amount)} de IGV)`}
                                    {saleLoaded?.perception_amount > 0 &&
                                        ` + ${Number2Currency(saleLoaded?.perception_amount)} (percepción)`}
                                    {saleLoaded?.packaging_amount > 0 &&
                                        ` + ${Number2Currency(saleLoaded?.packaging_amount)} (empaque)`}
                                    {saleLoaded?.additional_shipping_cost > 0 &&
                                        ` + ${Number2Currency(saleLoaded?.additional_shipping_cost)} (costo adicional)`}
                                    {saleLoaded?.seguro_importacion_total > 0 &&
                                        ` + ${Number2Currency(saleLoaded?.seguro_importacion_total)} (seguro)`}
                                    {saleLoaded?.derecho_arancelario_total >
                                        0 &&
                                        ` + ${Number2Currency(saleLoaded?.derecho_arancelario_total)} (derecho arancelario)`}
                                    {saleLoaded?.flete_total > 0 &&
                                        ` + ${Number2Currency(saleLoaded?.flete_total)} (flete)`}
                                    {saleLoaded?.promotion_discount > 0 &&
                                        ` - ${Number2Currency(saleLoaded?.promotion_discount)} (promociones)`}
                                    {saleLoaded?.coupon_discount > 0 &&
                                        ` - ${Number2Currency(saleLoaded?.coupon_discount)} (cupón)`}
                                    {saleLoaded?.bundle_discount > 0 &&
                                        ` - ${Number2Currency(saleLoaded?.bundle_discount)} (paquete)`}
                                    {saleLoaded?.renewal_discount > 0 &&
                                        ` - ${Number2Currency(saleLoaded?.renewal_discount)} (renovación)`}
                                    = {CurrencySymbol()}{" "}
                                    {Number2Currency(totalAmount)}
                                </small>
                            </div>
                            ...
                            >
                                ...
                                <div>
                                    <SelectFormGroup
                                        ...
                                        onChange={(e) =>
                                            onStatusChange(e, saleLoaded)
                                        }
                                        value={saleLoaded?.status_id}
                                        changeWith={[saleLoaded]}
                                        disabled={
                                            statusLoading ||
                ...
                <div className="row">
                    <div className="col-12">
                        <p className="text-muted mb-4">
                            Selecciona los campos que deseas incluir en la
                            exportación de Excel. Los campos están organizados
                            por categorías para facilitar su selección:
                        </p>

                        {Object.entries(exportableFieldsCategories).map(
                            ([categoryName, fields]) => (
