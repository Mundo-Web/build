...
import SectorsRest from "../Actions/Admin/SectorsRest";
...
const sectorsRest = new SectorsRest();
...
const Sectors = () => {
    ...
    const onModalSubmit = async (e) => {
        ...
        const result = await sectorsRest.save(formData);
        if (!result) return;
    ...
    const onVisibleChange = async ({ id, value }) => {
        const result = await sectorsRest.boolean({
            id,
            field: "visible",
            value,
        });
    ...
    const onDeleteClicked = async (id) => {
        ...
        const result = await sectorsRest.delete(id);
        if (!result) return;
        $(gridRef.current).dxDataGrid("instance").refresh();
    };
    ...
    const onReorder = async (e) => {
        ...
        try {
            const result = await sectorsRest.reorder(
                e.itemData.id,
                newOrderIndex,
            );
            if (result) {
            ...
            console.error("Error reordering sector:", error);
        }
    };
        ...
        <>
            <Table
                ...
                columns={[
                        ...
                        cellTemplate: (container, { data }) => {
                            if (data.image) {
                                ReactAppend(
                                    container,
                                    <img
                                        src={`/storage/images/sector/${data.image}`}
                                        alt={data.name}
                                        style={{
                                            width: "70px",
                                            height: "70px",
                    ...
                    Fillable.has("sectors", "background_image") && {
                        ...
                        cellTemplate: (container, { data }) => {
                            if (data.background_image) {
                                ReactAppend(
                                    container,
                                    <img
                                        src={`/storage/images/sector/${data.background_image}`}
                                        alt={data.name}
                                        style={{
                                            width: "70px",
                                            height: "70px",
                    ...
                    Fillable.has("sectors", "name") && {
                        dataField: "name",
                        caption: "Nombre",
                        width: "25%",
                    },
                    Fillable.has("sectors", "description") && {
                        dataField: "description",
                        caption: "Descripción",
                        width: "40%",
                    },
                    Fillable.has("sectors", "slug") && {
                        dataField: "slug",
                        caption: "Slug",
                        width: "15%",
                    },
            ...
            >
                ...
                <div className="row" id="sectors-container">
                                ...
                                <div className="row">
                                    <div className="col-12">
                                        <InputFormGroup
                                            ...
                                            placeholder="Ej: Minería, Industria, Construcción"
                                            required
                                        />
                                    </div>
                                    <div className="col-12">
