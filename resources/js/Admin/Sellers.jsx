...
const Sellers = ({}) => {
    ...
    const onModalOpen = (data) => {
        ...
        // Always force Seller role
        if (roleRef.current) roleRef.current.value = "Seller";
    ...
    const onModalSubmit = async (e) => {
        ...
        const result = await sellersRest.save(request);
        if (!result) return;
    ...
    const onInviteSubmit = async (e) => {
        ...
        const result = await sellersRest.invite({ email });
        if (!result) return;
    ...
    const onDeleteClicked = async (id) => {
        ...
        const result = await sellersRest.delete(id);
        if (!result) return;
        $(gridRef.current).dxDataGrid("instance").refresh();
    };
    ...
    const onStatusChange = async ({ id, value }) => {
        const { isConfirmed } = await Swal.fire({
            title: value ? "Activar vendedor" : "Desactivar vendedor",
            text: `¿Estás seguro de ${value ? "activar" : "desactivar"} este vendedor?`,
            icon: "question",
            showCancelButton: true,
        ...
        const result = await sellersRest.boolean({
            id,
            field: "status",
            value,
        });
    ...
    const onVaultOpen = async (data) => {
        ...
        const result = await sellersRest.getVault(data.id);
    ...
    const onVaultSubmit = async (e) => {
        ...
        const result = await sellersRest.updateVault(request);
        if (result) {
            const updatedVault = await sellersRest.getVault(
                selectedSeller.id,
            );
    ...
    const onDeleteVaultItem = async (id) => {
        const { isConfirmed } = await Swal.fire({
            ...
            text: "Se eliminará este artículo de la bóveda del vendedor.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonText: "Sí, eliminar",
        });
        ...
        const result = await sellersRest.deleteVaultItem(id);
        if (result) {
            const updatedVault = await sellersRest.getVault(
                selectedSeller.id,
            );
    ...
    return (
        <div className="container-fluid">
                                            ...
                                            if (data.status == 1) {
                                                ...
                                                const { isConfirmed } =
                                                    await Swal.fire({
                                                        ...
                                                        text: "El vendedor se desactivará y pasará a estado 'Archivado'.",
                                                        icon: "warning",
                                                        showCancelButton: true,
                                                        confirmButtonText:
                                                            "Sí, archivar",
            ...
            <Modal
                modalRef={modalRef}
                title={isEditing ? "Editar vendedor" : "Crear nuevo vendedor"}
            ...
            >
                <div className="row" id="principal-container">
                    ...
                    {/* Campo oculto para asegurar que se cree como Seller */}
                    <input type="hidden" ref={roleRef} value="Seller" />
                ...
                <div className="row">
                    <div className="col-12 mb-4">
                        <p className="text-muted">
                            ...
                            completar su registro como vendedor.
                        </p>
                    </div>
                    <InputFormGroup
                        eRef={inviteEmailRef}
            ...
            <Modal
                ...
                title={`Bóveda de ${selectedSeller?.name || "Vendedor"}`}
                hideSave
                size="lg"
            >
                <div>
...
CreateReactScript((el, properties) => {
    createRoot(el).render(
        <BaseAdminto {...properties} title="Vendedores">
            <Sellers {...properties} />
        </BaseAdminto>,
    );
});
