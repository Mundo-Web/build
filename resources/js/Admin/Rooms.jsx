...
const Rooms = () => {
    ...
    const onModalOpen = async (data) => {
        ...
        if (descriptionRef.editor) {
            SetQuillValue(descriptionRef.editor, data?.description ?? "");
    ...
    const onModalSubmit = async (e) => {
        ...
        const request = {
            ...
            description: descriptionRef.current.getValue
                ? descriptionRef.current.getValue()
        ...
        // Limpiar estados
        // Limpiar estados y campos
        setGallery([]);
        setPdfs([]);
        setVideos([]);
        setFeatures([]);
        if (descriptionRef.editor) {
            SetQuillValue(descriptionRef.editor, "");
        }
        idRef.current.value = "";
    ...
    return (
        <>
            <Table
                ...
                rest={itemsRest}
                restParams={{
                    filters: JSON.stringify([["type", "=", "room"]]),
                }}
                toolBar={(container) => {
                    container.unshift({
                        widget: "dxButton",
                ...
                columns={[
                        ...
                        cellTemplate: (container, { data }) => {
                            if (data.image) {
                                ...
                                ReactAppend(
                                    ...
                                    >
                                        <i
                                            className="mdi mdi-bed-double text-muted"
                                            style={{ fontSize: "32px" }}
                                        ></i>
                                    </div>,
                                );
                          ...
                          },
                    {
                        ...
                        cellTemplate: (container, { data }) => {
                            ReactAppend(
                                container,
                                <div className="small text-muted line-clamp-3">
                                    <div
                                ...
                                <div>
                                    ...
                                    {Fillable.has("items", "beds_count") && (
                                        <div className="small">
                                            ...
                                            {data.beds_count || 0} cama
                                            {data.beds_count !== 1 ? "s" : ""}
                                        </div>
                                    )}
                                    {Fillable.has("items", "size_m2") &&
                                        data.size_m2 > 0 && (
                                            <div className="small">
...
CreateReactScript((el, properties) => {
    createRoot(el).render(
        <BaseAdminto {...properties} title="Habitaciones">
            <Rooms {...properties} />
        </BaseAdminto>,
    );
});
