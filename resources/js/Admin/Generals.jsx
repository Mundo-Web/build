...
const Generals = ({
    ...
    const shouldShowTab = useCallback(
        (tabName) => {
            ...
            return correlatives.some((correlative) =>
                shouldShowField(correlative),
            );
        },
        [shouldShowField],
    );
    ...
    const shouldShowFieldRef = useCallback(
        (correlative) => {
    ...
    const emailTemplates = useMemo(() => {
        return generals.filter(
            (g) =>
                ...
                !g.correlative.startsWith("visibility_") &&
                g.correlative !== "support_email" &&
                g.correlative !== "coorporative_email",
        );
    }, [generals]);

    const [showPreview, setShowPreview] = useState(false);
    const [selectedEmailCorrelative, setSelectedEmailCorrelative] =
        useState("");
            ...
            if (allGenerals && allGenerals.length > 0) {
                allGenerals.forEach((general) => {
                    if (general && general.correlative) {
                        visibility[general.correlative] =
                            general.status === 1 || general.status === "1";
                    }
                });
            }
    ...
    const shouldShowSeoField = useCallback(
        (correlative) => {
    ...
    const [formData, setFormData] = useState(() => ({
        ...
        email_templates: Object.fromEntries(
            allEmailTemplateCorrelatives.map((correlative) => {
                const existing = generals.find(
                    (g) => g.correlative === correlative,
                );
        ...
        variable_visibility: Object.fromEntries(
            allEmailTemplateCorrelatives.map((correlative) => {
                const existing = generals.find(
                    (g) => g.correlative === `visibility_${correlative}`,
                );
                try {
        ...
        emails: generals
            ...
            ?.map((x) => x.trim()) ?? [""],
        cintillos: (() => {
            const cintilloGeneral = generals.find(
                (x) => x.correlative == "cintillo",
            );
      ...
      generals.find((x) => x.correlative == "footer_description")
        ...
        headerMenuOrder:
            generals.find((x) => x.correlative == "header_menu_order")
                ?.description ?? "",
        whatsappAdvisors: (() => {
            const advisorsGeneral = generals.find(
        ...
        shippingFree:
            generals.find((x) => x.correlative == "shipping_free")
                ?.description ?? "",
        currency:
            generals.find((x) => x.correlative == "currency")?.description ??
            "",
        exchange_rate_usd_pen:
            generals.find((x) => x.correlative == "exchange_rate_usd_pen")
                ?.description ?? "3.75",
        additional_shipping_costs: (() => {
            const costs = generals.find(
                (x) => x.correlative == "additional_shipping_costs",
            );
        ...
        checkout_culqi:
            generals.find((x) => x.correlative == "checkout_culqi")
                ?.description ?? "",
        checkout_culqi_name:
            generals.find((x) => x.correlative == "checkout_culqi_name")
                ?.description ?? "",
        checkout_culqi_public_key:
            generals.find((x) => x.correlative == "checkout_culqi_public_key")
                ?.description ?? "",
        checkout_culqi_private_key:
            generals.find((x) => x.correlative == "checkout_culqi_private_key")
                ?.description ?? "",
        checkout_culqi_rsa_id:
            generals.find((x) => x.correlative == "checkout_culqi_rsa_id")
                ?.description ?? "",
        checkout_culqi_rsa_public_key:
            generals.find(
                (x) => x.correlative == "checkout_culqi_rsa_public_key",
            )?.description ?? "",
        checkout_culqi_supports_usd:
            generals.find((x) => x.correlative == "checkout_culqi_supports_usd")
                ?.description ?? "false",
        checkout_culqi_commission:
        ...
        checkout_mercadopago:
            generals.find((x) => x.correlative == "checkout_mercadopago")
                ?.description ?? "",
        checkout_mercadopago_name:
            generals.find((x) => x.correlative == "checkout_mercadopago_name")
                ?.description ?? "",
        checkout_mercadopago_public_key:
            generals.find(
                (x) => x.correlative == "checkout_mercadopago_public_key",
            )?.description ?? "",
        checkout_mercadopago_private_key:
            generals.find(
                (x) => x.correlative == "checkout_mercadopago_private_key",
            )?.description ?? "",
        checkout_mercadopago_commission:
            generals.find(
                (x) => x.correlative == "checkout_mercadopago_commission",
            )?.description ?? "",
        checkout_openpay:
            generals.find((x) => x.correlative == "checkout_openpay")
                ?.description ?? "",
        checkout_openpay_name:
            generals.find((x) => x.correlative == "checkout_openpay_name")
                ?.description ?? "",
        checkout_openpay_merchant_id:
            generals.find(
                (x) => x.correlative == "checkout_openpay_merchant_id",
            )?.description ?? "",
        checkout_openpay_public_key:
            generals.find((x) => x.correlative == "checkout_openpay_public_key")
                ?.description ?? "",
        checkout_openpay_private_key:
            generals.find(
                (x) => x.correlative == "checkout_openpay_private_key",
            )?.description ?? "",
        checkout_openpay_commission:
            generals.find((x) => x.correlative == "checkout_openpay_commission")
                ?.description ?? "",
        checkout_openpay_sandbox_mode:
            generals.find(
                (x) => x.correlative == "checkout_openpay_sandbox_mode",
            )?.description ?? "false",
        checkout_dwallet:
            generals.find((x) => x.correlative == "checkout_dwallet")
                ?.description ?? "",
        checkout_dwallet_qr:
            generals.find((x) => x.correlative == "checkout_dwallet_qr")
                ?.description ?? "",
        checkout_dwallet_name:
            generals.find((x) => x.correlative == "checkout_dwallet_name")
                ?.description ?? "",
        checkout_dwallet_description:
            generals.find(
                (x) => x.correlative == "checkout_dwallet_description",
            )?.description ?? "",
        checkout_dwallet_commission:
            generals.find((x) => x.correlative == "checkout_dwallet_commission")
                ?.description ?? "",
        checkout_transfer:
            generals.find((x) => x.correlative == "checkout_transfer")
                ?.description ?? "",
        checkout_transfer_cci:
            generals.find((x) => x.correlative == "checkout_transfer_cci")
                ?.description ?? "",
        checkout_transfer_name:
            generals.find((x) => x.correlative == "checkout_transfer_name")
                ?.description ?? "",
        checkout_transfer_description:
            generals.find(
                (x) => x.correlative == "checkout_transfer_description",
            )?.description ?? "",
        checkout_transfer_commission:
            generals.find(
                (x) => x.correlative == "checkout_transfer_commission",
            )?.description ?? "",
        transfer_accounts: generals.find(
            (x) => x.correlative == "transfer_accounts",
        )?.description
            ? JSON.parse(
                  generals.find((x) => x.correlative == "transfer_accounts")
                      ?.description,
              )
            : [
                  {
        ...
        importation_flete:
            generals.find((x) => x.correlative == "importation_flete")
                ?.description ?? "",
        importation_seguro:
            generals.find((x) => x.correlative == "importation_seguro")
                ?.description ?? "",
        importation_derecho_arancelario:
            generals.find(
                (x) => x.correlative == "importation_derecho_arancelario",
            )?.description ?? "",
        importation_derecho_arancelario_descripcion:
            generals.find(
                (x) =>
        ...
        tiktokPixelId:
            generals.find((x) => x.correlative == "tiktok_pixel_id")
                ?.description ?? "",
        hotjarId:
            generals.find((x) => x.correlative == "hotjar_id")?.description ??
            "",
        clarityId:
            generals.find((x) => x.correlative == "clarity_id")?.description ??
            "",
        linkedinInsightTag:
        ...
        // Campos SEO
        siteTitle:
            generals.find((x) => x.correlative == "site_title")?.description ??
            "",
        siteDescription:
        ...
        ogDescription:
            generals.find((x) => x.correlative == "og_description")
                ?.description ?? "",
        ogImage:
            generals.find((x) => x.correlative == "og_image")?.description ??
            "",
        ogUrl:
            generals.find((x) => x.correlative == "og_url")?.description ?? "",
        twitterTitle:
        ...
        footerCompanyLinks: (() => {
            try {
                const links = generals.find(
                    (x) => x.correlative == "footer_company_links",
                )?.description;
    ...
    const handleSupportPhoneChange = useCallback(
        (e) => {
    ...
    const handleCoorporativeEmailChange = useCallback(
        (e) => {
    ...
    const handleSupportEmailChange = useCallback(
        (e) => {
    ...
    const handlePhoneWhatsappChange = useCallback(
        (e) => {
    ...
    const handleMessageWhatsappChange = useCallback(
        (e) => {
    ...
    const handleCheckoutCulqiChange = useCallback(
        (e) => {
    ...
    const handleCheckoutMercadopagoChange = useCallback(
        (e) => {
    ...
    const handleCheckoutOpenpayChange = useCallback(
        (e) => {
    ...
    const handleOpenpayNameChange = useCallback(
        (e) => {
    ...
    const handleOpenpayMerchantIdChange = useCallback(
        (e) => {
    ...
    const handleOpenpayPublicKeyChange = useCallback(
        (e) => {
    ...
    const handleOpenpayPrivateKeyChange = useCallback(
        (e) => {
    ...
    const handleCheckoutDwalletChange = useCallback(
        (e) => {
    ...
    const handleCheckoutTransferChange = useCallback(
        (e) => {
    ...
    const handleCulqiNameChange = useCallback(
        (e) => {
    ...
    const handleCulqiPublicKeyChange = useCallback(
        (e) => {
    ...
    const handleCulqiPrivateKeyChange = useCallback(
        (e) => {
    ...
    const handleCulqiRsaIdChange = useCallback(
        (e) => {
    ...
    const handleCulqiRsaPublicKeyChange = useCallback(
        (e) => {
    ...
    const handleSaveVisibility = async () => {
        ...
        try {
            ...
            const visibilityUpdates = Object.entries(fieldVisibility).map(
                ([correlative, isVisible]) => ({
                    correlative,
            ...
            const response =
                await generalsRest.updateVisibility(visibilityUpdates);
            ...
            if (response.success) {
                ...
                toast.error(
                    response.message ||
                        "Error al actualizar la visibilidad de campos",
                );
            }
        } catch (error) {
    ...
    const getActiveStatus = (cintillo) => {
        ...
        const enabled =
            typeof cintillo === "string" ? true : cintillo.enabled !== false;

        if (!enabled || !text?.trim())
            return { status: "Inactivo", class: "badge bg-secondary" };

        if (typeof cintillo === "string")
            return { status: "Activo", class: "badge bg-success" };
        ...
        if (!daySchedule || !daySchedule.enabled)
            return {
                status: "Programado (Día no habilitado)",
        ...
        if (isActiveNow) {
            ...
            return {
                status: "Programado (Fuera de horario)",
                class: "badge bg-warning",
            };
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const dataToSend = [
            ...
            ...Object.keys(formData.email_templates).map((correlative) => ({
                correlative,
                name:
                    emailTemplates.find((t) => t.correlative === correlative)
                        ?.name || correlative,
            ...
            },
            {
                ...
                description:
                    formData.importation_derecho_arancelario_descripcion || "",
            },
            {
            ...
            ...allEmailTemplateCorrelatives.map((correlative) => ({
                ...
                description: formData.variable_visibility[correlative]
                    ? JSON.stringify(formData.variable_visibility[correlative])
                    : "",
            })),
            {
                        ...
                        >
                            <div className="mb-2">
                                    ...
                                    {emailTemplates.map((t) => {
                                        const friendlyNames = {
                                            ...
                                            blog_published_email:
                                                "Blog Publicado",
                                            claim_email:
                                                "Reclamo Recibido (Cliente)",
                                            whistleblowing_email:
                                                "Denuncia Recibida (Cliente)",
                                            password_changed_email:
                                                "Contraseña Cambiada",
                                            reset_password_email:
                                ...
                                <TinyMCEFormGroup
                                    ...
                                    label={
                                        <>
                                            Plantilla de Email (HTML seguro,
                                            variables:{" "}
                                            <code>{`{{variable}}`}</code>)
                                        </>
                                    }
                                                        ...
                                                        return (
                                                            <Tippy
                                                                content={
                                                                    hasRootRole()
                                                                        ? !isVisible
                                                                            ? "Variable Oculta para Admins"
                                                                            : "Variable Visible"
                                                                        : `Haz clic para copiar: {{${key}}}`
                                                                }
                                                                key={key}
                                                            >
                                                                <div
                                                                ...
                                                                >
                                                                                ...
                                                                                onChange={(
                                                                                    ...
                                                                                    const currentVisibility =
                                                                                        formData
                                                                                            ...
                                                                                            : Object.keys(
                                                                                                  templateVariables,
                                                                                              ); // Si es null (todo visible), init con todo para empezar a ocultar
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <span
                                                                            ...
                                                                            className={`ms-1 border-start ps-1 ${!isVisible ? "text-white-50" : "text-muted"} small`}
                                                                        >
                                                                            {
                                                                                desc
                                                                            }
                        ...
                        >
                            <div className="row">
                                <ConditionalField correlative="cintillo">
                                    <div className="col-md-6 mb-2">
                                        <div className="alert alert-info">
                                            ...
                                            <p className="mb-2">
                                                Los cintillos se han movido a
                                                una pestaña dedicada para una
                                                mejor organización.
                                            </p>
                                            <button
                                                type="button"
                                ...
                                <ConditionalField correlative="copyright">
                                    <div
                                        className={`mb-2 ${shouldShowField("cintillo") ? "col-md-6 " : "col-md-12 "}`}
                                    >
                                        <label
                                            htmlFor="copyright"
                            ...
                            <ConditionalField correlative="excel_import_template">
                                <div className="mb-2">
                                    ...
                                    {formData.excelImportTemplate ? (
                                        <div className="position-relative">
                                            ...
                                            <div className="d-flex align-items-center p-3 border rounded bg-light">
                                                <i
                                                    className="mdi mdi-file-excel text-success me-2"
                                                    style={{ fontSize: "2rem" }}
                                                ></i>
                                                <div className="flex-grow-1">
                                                    <strong>
                                                        Plantilla Excel cargada
                                                    </strong>
                                                    <br />
                                        ...
                                        <input
                                            ...
                                            onChange={async (e) => {
                                                ...
                                                if (
                                                    ...
                                                    alert(
                                                        "Por favor, selecciona solo archivos Excel (.xlsx o .xls)",
                                                    );
                                                    e.target.value = null;
                                                    return;
                                                }
                                    ...
                                    <small className="text-muted">
                                        Esta plantilla se descargará desde el
                                        modal de importación en Items. Solo
                            ...
                            <ConditionalField correlative="header_menu_order">
                                <div className="mb-2">
                                    ...
                                    />
                                    <small className="text-muted">
                                        Nombres de categorías y páginas
                                        separados por comas. Use los nombres
                                        exactos como aparecen en el menú.
                                    </small>
                                </div>
                            </ConditionalField>
                            ...
                            <ConditionalField correlative="footer_company_links">
                                <div className="mb-2">
                                    ...
                                    {formData.footerCompanyLinks.map(
                                        (link, index) => (
                                            <div
                                                key={index}
                                                className="card mb-2"
                                                    ...
                                                    <div className="row">
                                                        ...
                                                        <div className="col-md-1 d-flex gap-1">
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    const newLinks =
                                                                        formData.footerCompanyLinks.filter(
                                                                            (
                                                                                _,
                                                                                i,
                                                                            ) =>
                                    ...
                                    <small className="text-muted d-block mt-2">
                                        Estos enlaces aparecerán en la sección
                                        del footer
                                    </small>
                                </div>
                            </ConditionalField>
                            ...
                            <ConditionalField correlative="body_custom_html">
                                <div className="mb-2">
                                    ...
                                    <small className="text-muted d-block mt-1">
                                        HTML que se insertará justo después del
                                        &lt;body&gt;. Útil para overlays
                                        decorativos, gradientes de fondo,
                                        ...
                                        para overlays que no bloqueen la
                                        interacción.
                                    </small>
                                </div>
                            </ConditionalField>
                        ...
                        >
                            ...
                            <ConditionalField correlative="coorporative_email">
                                <div className="mb-2">
                                    <label
                                        ...
                                        className="form-label"
                                    >
                                        Correo corporativo (aquí llegaran los
                                        emails salientes)
                                    </label>
                                    <input
                                        type="email"
                            ...
                            >
                                ...
                                <p className="text-muted small mb-3">
                                    Agrega múltiples asesores. Si hay más de
                                    uno, se mostrará un modal para que el
                                    cliente elija.
                                </p>
                                    ...
                                    >
                                        ...
                                        {some(
                                            ...
                                            <a
                                                ...
                                                id="v-mercadopago-tab"
                                                data-bs-toggle="pill"
                                                href="#v-mercadopago"
                                                role="tab"
                                                aria-controls="v-mercadopago"
                                                aria-selected="false"
                                            >
                                                Mercado Pago
                                            </a>
                                        ...
                                        {some(
                                            ...
                                            <a
                                                className="nav-link mb-1"
                                                id="v-digital-wallet-tab"
                                                data-bs-toggle="pill"
                                                href="#v-digital-wallet"
                                                role="tab"
                                                aria-controls="v-digital-wallet"
                                                aria-selected="false"
                                            >
                                                Yape / Plin
                                            </a>
                                ...
                                <div className="tab-content col-sm-9">
                                        ...
                                        >
                                                ...
                                                <div className="mb-2">
                                                    <div className="form-check">
                                                        <input
                                                            ...
                                                            id="checkout-culqi"
                                                            checked={
                                                                formData.checkout_culqi ==
                                                                "true"
                                                            }
                                                            onChange={
                                                                handleCheckoutCulqiChange
                                                            }
                                                        />
                                                        <label
                                                            className="form-check-label form-label"
                                                            htmlFor="checkout-culqi"
                                                        >
                                                            Habilitar pago con
                                                            Culqi
                                            ...
                                            <ConditionalField correlative="checkout_culqi_name">
                                                <div className="mb-2">
                                                    ...
                                                    <input
                                                        ...
                                                        className="form-control"
                                                        value={
                                                            formData.checkout_culqi_name
                                                        }
                                                        onChange={
                                                            handleCulqiNameChange
                                                        }
                                                    />
                                                </div>
                                            </ConditionalField>
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_culqi_public_key
                                                    }
                                                    onChange={
                                                        handleCulqiPublicKeyChange
                                                    }
                                                />
                                            </div>
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_culqi_private_key
                                                    }
                                                    onChange={
                                                        handleCulqiPrivateKeyChange
                                                    }
                                                />
                                            </div>
                                            <div className="mt-4 mb-2" hidden>
                                                <h6 className="text-muted">
                                                    Configuración RSA (Custom
                                                    Checkout v4)
                                                </h6>
                                                <small className="text-muted d-block mb-2">
                                                    Estas claves RSA son
                                                    necesarias para el nuevo
                                                    Custom Checkout de Culqi.
                                                    Obténlas desde el panel de
                                                    Culqi en la sección de
                                                    claves RSA.
                                                </small>
                                            </div>
                                            <div className="mb-2" hidden>
                                                ...
                                                <input
                                                    ...
                                                    value={
                                                        formData.checkout_culqi_rsa_id
                                                    }
                                                    onChange={
                                                        handleCulqiRsaIdChange
                                                    }
                                                />
                                            </div>
                                            <div className="mb-2" hidden>
                                                ...
                                                <textarea
                                                    ...
                                                    value={
                                                        formData.checkout_culqi_rsa_public_key
                                                    }
                                                    onChange={
                                                        handleCulqiRsaPublicKeyChange
                                                    }
                                                />
                                                <small className="text-muted">
                                                    Copia la clave RSA pública
                                                    completa desde el panel de
                                                    Culqi
                                                </small>
                                            </div>
                                            <div className="mt-4 mb-2">
                                                ...
                                                <small className="text-muted d-block mb-2">
                                                    Configura si tu cuenta de
                                                    Culqi soporta pagos en
                                                    dólares (USD).
                                                </small>
                                            </div>
                                            <div className="mb-2">
                                                <div className="form-check">
                                                    <input
                                                        ...
                                                        checked={
                                                            formData.checkout_culqi_supports_usd ===
                                                            "true"
                                                        }
                                                        onChange={(e) =>
                                                            setFormData({
                                                    ...
                                                    <label
                                                        className="form-check-label form-label"
                                                        htmlFor="checkout-culqi-supports-usd"
                                                    >
                                                        Mi cuenta Culqi soporta
                                                        USD
                                                        <small className="text-muted d-block">
                                            ...
                                            <div className="mt-4 mb-2">
                                                ...
                                                <small className="text-muted d-block mb-2">
                                                    Configura la comisión (en
                                                    porcentaje) que se añadirá
                                                    al total cuando el cliente
                                                    elija este método de pago.
                                                </small>
                                            </div>
                                            <div className="mb-2">
                                                <label className="form-label">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Ej: 3.5"
                                                    value={
                                                        formData.checkout_culqi_commission
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                    ...
                                    <ConditionalField correlative="checkout_mercadopago">
                                        <div
                                            ...
                                            id="v-mercadopago"
                                            role="tabpanel"
                                            aria-labelledby="v-mercadopago-tab"
                                        >
                                            <div className="mb-2">
                                                <div className="form-check">
                                                    <input
                                                        ...
                                                        checked={
                                                            formData.checkout_mercadopago ==
                                                            "true"
                                                        }
                                                        onChange={
                                                            handleCheckoutMercadopagoChange
                                                        }
                                                    />
                                                    <label
                                                        className="form-check-label form-label"
                                                        htmlFor="checkout-mercadopago"
                                                    >
                                                        Habilitar pago con
                                                        Mercado Pago
                                                        <small className="text-muted d-block">
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_mercadopago_name
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_mercadopago_public_key
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                            ...
                                            <div className="mb-2">
                                                <label className="form-label">
                                                    Clave Privada (Access Token)
                                                </label>
                                                <input
                                                    type="password"
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_mercadopago_private_key
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Ej: 3.5"
                                                    value={
                                                        formData.checkout_mercadopago_commission
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                        ...
                                        >
                                            <div className="mb-2">
                                                <div className="form-check">
                                                    <input
                                                        ...
                                                        id="checkout-openpay"
                                                        checked={
                                                            formData.checkout_openpay ==
                                                            "true"
                                                        }
                                                        onChange={
                                                            handleCheckoutOpenpayChange
                                                        }
                                                    />
                                                    <label
                                                        className="form-check-label form-label"
                                                        htmlFor="checkout-openpay"
                                                    >
                                                        Habilitar pago con
                                                        OpenPay
                                                        <small className="text-muted d-block">
                                            ...
                                            <div className="mb-2">
                                                <div className="form-check">
                                                    <input
                                                        ...
                                                        checked={
                                                            formData.checkout_openpay_sandbox_mode ==
                                                            "true"
                                                        }
                                                        onChange={(e) =>
                                                            setFormData({
                                                    ...
                                                    <label
                                                        className="form-check-label form-label"
                                                        htmlFor="checkout-openpay-sandbox"
                                                    >
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_openpay_name
                                                    }
                                                    onChange={
                                                        handleOpenpayNameChange
                                                    }
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_openpay_merchant_id
                                                    }
                                                    onChange={
                                                        handleOpenpayMerchantIdChange
                                                    }
                                                    placeholder="mxxxxxxxxxxx"
                                                />
                                                <small className="text-muted">
                                                    El Merchant ID proporcionado
                                                    por OpenPay
                                                </small>
                                            </div>
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_openpay_public_key
                                                    }
                                                    onChange={
                                                        handleOpenpayPublicKeyChange
                                                    }
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_openpay_private_key
                                                    }
                                                    onChange={
                                                        handleOpenpayPrivateKeyChange
                                                    }
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Ej: 3.5"
                                                    value={
                                                        formData.checkout_openpay_commission
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                    ...
                                    <ConditionalField correlative="checkout_dwallet">
                                        <div
                                            ...
                                            id="v-digital-wallet"
                                            role="tabpanel"
                                            aria-labelledby="v-digital-wallet-tab"
                                        >
                                            <div className="mb-2">
                                                <div className="form-check">
                                                    <input
                                            ...
                                            <div className="mb-2">
                                                ...
                                                {formData.checkout_dwallet_qr ? (
                                                    <div className="position-relative">
                                                        ...
                                                        <img
                                                            src={`/assets/resources/${formData.checkout_dwallet_qr}`}
                                                            className="img-thumbnail"
                                                            style={{
                                                                height: "200px",
                                                                width: "auto",
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_dwallet_name
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <textarea
                                                    className="form-control"
                                                    value={
                                                        formData.checkout_dwallet_description
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Ej: 3.5"
                                                    value={
                                                        formData.checkout_dwallet_commission
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                      ...
                      />
                                    ...
                                    <ConditionalField correlative="checkout_transfer">
                                        <div
                                            ...
                                            id="v-transfer"
                                            role="tabpanel"
                                            aria-labelledby="v-transfer-tab"
                                        >
                                            <div className="mb-2">
                                                <div className="form-check">
                                                    <input
                                            ...
                                            {formData.transfer_accounts.map(
                                                (account, index) => (
                                                    <div
                                                        key={index}
                                            ...
                                            <div className="mb-2">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Ej: 3.5"
                                                    value={
                                                        formData.checkout_transfer_commission
                                                    }
                                                    onChange={(e) =>
                                                        setFormData({
                                                            ...formData,
                        ...
                        >
                            ...
                            >
                                <label className="form-label">
                                    Política del sistema de Gestión (PDF)
                                </label>
                                {formData.politicaSistemaGestion ? (
                                    <div className="position-relative">
                                        ...
                                        <div className="d-flex align-items-center p-3 border rounded bg-light">
                                            <i
                                                className="mdi mdi-file-pdf-box text-danger me-2"
                                                style={{ fontSize: "2rem" }}
                                            ></i>
                                            <div className="flex-grow-1">
                                                <strong>
                            ...
                            >
                                <label className="form-label">
                                    Alcance del sistema de Gestión (PDF)
                                </label>
                                ...
                                <div className="col-12">
                                    ...
                                    {formData.cintillos.length === 0 ? (
                                        <div className="text-center py-5">
                                            <i
                                                className="fas fa-bullhorn text-muted"
                                                style={{ fontSize: "4rem" }}
                                            ></i>
                                            <h5 className="text-muted mt-3">
                                                No hay cintillos configurados
                                            </h5>
                                            <p className="text-muted">
                                                Agrega tu primer cintillo para
                                                comenzar
                                            </p>
                                        </div>
                                    ) : (
                                                                                ...
                                                                                <div className="btn-group btn-group-sm">
                                                                                    ...
                                                                                    <button
                                                                                        ...
                                                                                        onClick={() => {
                                                                                            if (
                                                                                                confirm(
                                                                                                    "¿Estás seguro de que quieres eliminar este cintillo?",
                                                                                                )
                                                                                            ) {
                                                                                                handleRemoveField(
                                                                                                    index,
                            ...
                            >
                                <div className="mb-3">
                                    ...
                                    <small className="form-text text-muted">
                                        Escribe el nombre o dirección de tu
                                        tienda para centrar el mapa
                                        automáticamente.
                                    </small>
                                </div>
                                ...
                                <div className="mb-2">
                                    ...
                                    >
                                        ...
                                        <small className="d-block text-muted">
                                            Dejar en 0 si no se quiere mostrar
                                        </small>
                                    </label>
                                    <input
                                        type="number"
                                ...
                                <div className="mb-2">
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            Culqi solo procesa pagos en Soles
                                            (PEN). Este tipo de cambio se usará
                                            para convertir el monto de dólares a
                                            soles al procesar pagos con tarjeta.
                                        </small>
                                    </label>
                                    <div className="input-group">
                                        ...
                                        <input
                                            ...
                                            value={
                                                formData.exchange_rate_usd_pen
                                            }
                                            onChange={(e) =>
                                                setFormData({
                                                    ...formData,
                                    ...
                                    <small className="text-info">
                                        ...
                                        Ejemplo: Si un producto cuesta $100 USD
                                        y el tipo de cambio es 3.75, Culqi
                                        cobrará S/ 375.00 PEN.
                                    </small>
                                </div>
                            )}
                                ...
                                <div className="mb-4 mt-4">
                                    <div className="d-flex justify-content-between align-items-center mb-3">
                                        <div>
                                            ...
                                            <small className="text-muted">
                                                Configure costos adicionales
                                                según el método de envío y el
                                                monto de compra (Ej: embalaje,
                                                traslado a agencia, etc.)
                                            </small>
                                        </div>
                                        <button
                                            type="button"
                                                ...
                                                setFormData({
                                                    ...
                                                    additional_shipping_costs: [
                                                        ...formData.additional_shipping_costs,
                                                        {
                                                            ...
                                                            additional_cost: 15,
                                                            description:
                                                                "Costo de embalaje y traslado a agencia",
                                                        },
                                                    ],
                                                });
                                            }}
                                    ...
                                    {formData.additional_shipping_costs
                                        ...
                                        <div className="alert alert-info">
                                            <i className="mdi mdi-information me-2"></i>
                                            No hay reglas de costos adicionales
                                            configuradas. Haga clic en "Agregar
                                            Regla" para crear una.
                                        </div>
                                    ) : (
                                        <div className="row">
                                            {formData.additional_shipping_costs.map(
                                                (cost, index) => (
                                                    <div
                                                        key={index}
                                                                ...
                                                                <div className="d-flex align-items-center gap-2">
                                                                    <div className="form-check form-switch mb-0">
                                                                        ...
                                                                        <label
                                                                            ...
                                                                            htmlFor={`additional-cost-enabled-${index}`}
                                                                        >
                                                                            {cost.enabled
                                                                                ? "Activo"
                                                                                : "Inactivo"}
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        onClick={() => {
                                                                            if (
                                                                                confirm(
                                                                                    "¿Está seguro de eliminar esta regla?",
                                                                                )
                                                                            ) {
                                                                                const newCosts =
                                                                                    [
                                ...
                                <div className="col-12 col-sm-8 col-md-6 col-lg-4 col-xl-3">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <div className="input-group">
                                            <input
                                                ...
                                                value={
                                                    formData.importation_derecho_arancelario
                                                }
                                                onChange={(e) =>
                                                    setFormData({
                                                        ...formData,
                                    ...
                                    <div className="mb-3">
                                        ...
                                        >
                                            ...
                                            <small className="text-muted d-block">
                                                Qué porcentajes se están
                                                considerando en cálculo de
                                                derechos arancelarios
                                            </small>
                                        </label>
                                        <textarea
                        ...
                        >
                            <div className="mb-2">
                                ...
                                <small className="text-muted">
                                    ...
                                    corporativas y notificaciones del sistema
                                </small>
                            </div>
                        </div>
                        ...
                        >
                            <div className="row">
                                <div className="col-12">
                                    ...
                                    <p className="text-muted mb-4">
                                        Configure los meta tags para optimizar
                                        su sitio web para motores de búsqueda y
                                        redes sociales.
                                    </p>
                                </div>
                            </div>
                                ...
                                <div className="col-md-6">
                                    ...
                                    <ConditionalSeoField correlative="site_title">
                                        <div className="mb-3">
                                            ...
                                            />
                                            <small className="text-muted">
                                                Título principal que aparece en
                                    ...
                                    <ConditionalSeoField correlative="site_description">
                                        <div className="mb-3">
                                            ...
                                            />
                                            <small className="text-muted">
                                                Descripción que aparece en los
                                                resultados de búsqueda (máx. 160
                                                caracteres)
                                            </small>
                                        </div>
                                    </ConditionalSeoField>
                                ...
                                <div className="col-md-6">
                                    ...
                                    <ConditionalSeoField correlative="og_url">
                                        <div className="mb-3">
                                            ...
                                            />
                                            <small className="text-muted">
                                                URL que se mostrará al compartir
                                            </small>
                                        </div>
                                    </ConditionalSeoField>
                            ...
                            <div className="row mt-4">
                                ...
                                <div className="col-md-8">
                                    <ConditionalSeoField correlative="robots_additional_rules">
                                        <div className="mb-3">
                                            <label className="form-label">
                                                Reglas Adicionales de robots.txt
                                            </label>
                                            <textarea
                                                className="form-control"
                                                rows="8"
                                                ...
                                                value={
                                                    formData.robotsAdditionalRules
                                                }
                                                onChange={(e) =>
                                                    setFormData({
                                                        ...formData,
                                            ...
                                            <small className="text-muted">
                                                ...
                                                como Disallow adicionales o
                                                configuración de User-agents
                                                específicos. Ejemplo: "Disallow:
                                                /mi-ruta" o "User-agent:
                                                Bingbot\nCrawl-delay: 5"
                                            </small>
                                        </div>
                                    </ConditionalSeoField>
                                ...
                                <div className="col-12">
                                        ...
                                        <button
                                            ...
                                            onClick={async () => {
                                                try {
                                                    ...
                                                    const response =
                                                        await fetch(
                                                            "/api/admin/generals/generate-robots",
                                                            {
                                                                method: "POST",
                                                                headers: {
                                                                    "Content-Type":
                                        ...
                                        <button
                                            ...
                                            onClick={async () => {
                                                try {
                                                    ...
                                                    const response =
                                                        await fetch(
                                                            "/api/admin/generals/generate-sitemap",
                                                            {
                                                                method: "POST",
                                                                headers: {
                                                                    "Content-Type":
                                    ...
                                    <small className="text-muted mt-2 d-block">
                                        <i className="fas fa-info-circle me-1"></i>
                                        Recuerda guardar los cambios antes de
                                        generar los archivos para que se
                                        incluyan las nuevas configuraciones.
                                    </small>
                                </div>
                            </div>
                            ...
                            <div className="alert alert-info mt-4">
                                ...
                                <ul className="mb-0">
                                    <li>
                                        <strong>Título:</strong> Mantenga entre
                                        50-60 caracteres para mejor
                                        visualización
                                    </li>
                                    <li>
                                        <strong>Descripción:</strong> Use entre
                                        150-160 caracteres para aparecer
                                        completa en búsquedas
                                    </li>
                                    <li>
                                        <strong>Imágenes:</strong> Use imágenes
                                        de alta calidad (1200x630px para Open
                                        Graph)
                                    </li>
                                    <li>
                                        <strong>Palabras clave:</strong> Use
                                        términos relevantes que sus usuarios
                                        buscarían
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div className="row">
                                <div className="col-md-6">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        />
                                        <small className="text-muted">
                                            ID de Google Analytics para tracking
                                            de visitas
                                        </small>
                                    </div>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            className="form-control"
                                            value={
                                                formData.googleAdsConversionLabel
                                            }
                                            onChange={(e) =>
                                                setFormData({
                                                    ...formData,
                                    ...
                                    <div className="mb-3">
                                        ...
                                        />
                                        <small className="text-muted">
                                            ID del Insight Tag de LinkedIn
                                        </small>
                                    </div>
                                </div>
                                ...
                                <div className="col-md-6">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        />
                                        <small className="text-muted">
                                            ID de Hotjar para mapas de calor
                                        </small>
                                    </div>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        />
                                        <small className="text-muted">
                                            Scripts personalizados para el
                                            &lt;head&gt;
                                        </small>
                                    </div>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        />
                                        <small className="text-muted">
                                            Scripts personalizados para el final
                                            del &lt;body&gt;
                                        </small>
                                    </div>
                                </div>
                            ...
                            <div className="row">
                                <div className="col-12">
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <small className="text-muted">
                                            Permite a los clientes registrarse e
                                            iniciar sesión usando su cuenta de
                                            Google
                                        </small>
                                    </div>
                                        ...
                                        <>
                                            <div className="alert alert-info">
                                                ...
                                                <ol className="mb-0">
                                                    ...
                                                    <li>
                                                        Copia el Client ID y
                                                        Client Secret en los
                                                        campos de abajo
                                                    </li>
                                                </ol>
                                            </div>
                                            ...
                                            <div className="alert alert-warning">
                                                ...
                                                <p className="mb-0">
                                                    ...
                                                    dominios autorizados en
                                                    Google Cloud Console con tu
                                                    dominio real (ej:
                                                    https://tudominio.com)
                                                </p>
                                            </div>
                                    ...
                                    <ConditionalField correlative="tiny_api_key">
                                        <div className="card mb-4 border-light shadow-sm">
                                            ...
                                            <div className="card-body">
                                                <label
                                                    className="form-label"
                                                    htmlFor="tiny_api_key"
                                                >
                                                    TinyMCE API Key{" "}
                                                    <small className="text-muted">
                                                        (Dejar en blanco para
                                                        usar la key por defecto
                                                        del sistema)
                                                    </small>
                                                </label>
                                                <div className="input-group">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={
                                                            handleTinyApiKeyChange
                                                        }
                                                        placeholder="Ingrese su API Key de TinyMCE"
                                                    />
                                                </div>
                                                <small className="text-muted mt-2 d-block">
                                                    Esta key se utiliza para
                                    ...
                                    <ConditionalField correlative="atalaya_leads_api_key">
                                        <div className="card mb-4 border-light shadow-sm">
                                            ...
                                            <div className="card-body">
                                                ...
                                                <small className="text-muted mt-2 d-block">
                                                    API Key para enviar leads
                                                    del formulario de contacto a
                                                    Atalaya CRM.
                                                    <a
                                                        href="https://atalaya.pe"
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                            ...
                            <div className="row">
                                <div className="col-12">
                                    ...
                                    <div className="alert alert-info mb-4">
                                        ...
                                        Estos horarios definen cuándo los
                                        huéspedes pueden ingresar y deben
                                        retirarse de las habitaciones. El día de
                                        check-out, la habitación estará
                                        disponible para nuevas reservas a partir
                                        ...
                                        <div className="col-md-6">
                                            <div className="card">
                                                <div className="card-body">
                                                    ...
                                                    <p className="text-muted small mb-3">
                                                        Hora límite para que los
                                                        huéspedes desocupen la
                                                        habitación
                                                    </p>
                                                    <div className="mb-3">
                                                        <label className="form-label">
                                    ...
                                    <div className="alert alert-warning mt-4">
                                        ...
                                        <ul className="mb-0">
                                            <li>
                                                Si un huésped reserva del{" "}
                                                <strong>día 5 al día 8</strong>,
                                                el check-out es el{" "}
                                                <strong>día 8</strong>.
                                            </li>
                                            <li>
                                                El <strong>día 8</strong> estará
                                                disponible para nuevas reservas,
                                                ya que el huésped debe retirarse
                                                a las{" "}
                                                <strong>
                                                    {formData.hotelCheckoutTime ||
                                                        "12:00"}
                                            ...
                                            <li>
                                                Un nuevo huésped puede hacer
                                                check-in el mismo{" "}
                                                <strong>día 8</strong> a partir
                                                de las{" "}
                                                <strong>
                                                    {formData.hotelCheckinTime ||
                                                        "14:00"}
                            ...
                            <div className="row">
                                <div className="col-12">
                                    ...
                                    <div className="card mb-4">
                                        ...
                                        <div className="card-body">
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <small className="text-muted">
                                                    Permite a los clientes pagar
                                                    con tarjetas de
                                                    crédito/débito, Yape, y
                                                    otros métodos
                                                </small>
                                            </div>
                                    ...
                                    <div className="alert alert-info">
                                        ...
                                        <p className="mb-2">
                                            Las configuraciones de pasarelas de
                                            pago se almacenan de forma segura en
                                            la base de datos.
                                        </p>
                                        <ul className="mb-0">
                                            <li>
                                                Las llaves privadas nunca se
                                                exponen al frontend
                                            </li>
                                            <li>
                                                Las configuraciones se pueden
                                                cambiar sin editar archivos del
                                                servidor
                                            </li>
                                            <li>
                        ...
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <p className="text-muted mb-3">
                                    Selecciona qué campos serán visibles para
                                    los usuarios Admin. Los campos marcados
                                    estarán disponibles para Admin, los
                                    desmarcados solo serán visibles para Root.
                                </p>

                                {/* Organizar campos por categorías */}
                                {Object.entries(tabCorrelatives).map(
                                    ([tabKey, correlatives]) => {
                                        const tabNames = {
                                            general: "General",
                                                        ...
                                                        (general) => (
                                                            ...
                                                            >
                                                                <div className="form-check">
                                                                    ...
                                                                    <label
                                                                        className="form-check-label"
                                                                        htmlFor={`field-${general.correlative}`}
                                                                    >
                                                                        <strong>
                                                                            {
                                                                                general.name
                                        ...
                                        {[
                                            ...
                                            >
                                                <div className="form-check">
                                                    ...
                                                    <label
                                                        className="form-check-label"
                                                        htmlFor={`field-${field.correlative}`}
                                                    >
                                                        <strong>
                                                            {field.name}
                                                        </strong>
                                ...
                                {(() => {
                                    const allCategorizedCorrelatives =
                                        Object.values(tabCorrelatives).flat();
                                    const uncategorizedFields = (
                                        allGenerals || generals
                                    ).filter(
                                        (general) =>
                                            !allCategorizedCorrelatives.includes(
                                                general.correlative,
                                            ),
                                    );
                                    ...
                                    return (
                                        <div className="mb-4">
                                            <div className="d-flex align-items-center mb-3">
                                                <h4 className="text-secondary mb-0 me-2">
                                                    Otros Campos
                                                </h4>
                                            </div>
                                            <div className="row">
                                                {uncategorizedFields.map(
                                                    (general) => (
                                                        <div
                                                            key={
                                                                general.correlative
                        ...
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ...
                                <div className="mb-4">
                                    ...
                                    <small className="text-muted">
                                        <i className="fas fa-info-circle me-1"></i>
                                        Evita usar comas en el texto para mejor
                                        compatibilidad
                                    </small>
                                </div>
                                ...
                                <div className="mb-3">
                                    ...
                                    >
                                        {[
                                            ...
                                            const daySchedule = modalCintillo
                                                .schedule[day.key] || {
                                                enabled: true,
                                                start: "00:00",
                                                end: "23:59",
                                            ...
                                            return (
                                                <div
                                                    key={day.key}
                                                    className="row mb-3 align-items-center p-2 rounded"
                                                    style={{
                                                        backgroundColor:
                                                            daySchedule.enabled
                                                                ? "#DCFCE7"
