...
const Applications = () => {
  ...
  const onModalOpen = (data) => {
    ...
    if (imageRef.current && imageRef.image) {
      imageRef.image.src = data?.image ? `/storage/images/application/${data.image}` : '';
      imageRef.current.value = null;
    }
    if (backgroundImageRef.current && backgroundImageRef.image) {
      backgroundImageRef.image.src = data?.background_image ? `/storage/images/application/${data.background_image}` : '';
  ...
  const onVisibleChange = async ({ id, value }) => {
    const result = await applicationsRest.boolean({ id, field: 'visible', value });
    if (!result) return;
    $(gridRef.current).dxDataGrid('instance').refresh();
  };

  const onStatusChange = async ({ id, value }) => {
    const result = await applicationsRest.boolean({ id, field: 'status', value });
    if (!result) return;
    $(gridRef.current).dxDataGrid('instance').refresh();
  };
    ...
    <>
      ...
      >
        ...
        <div className="row" id="applications-container">
                ...
                <div className="row">
                  ...
                  <div className="col-12">
                    <TextareaFormGroup
                      ...
                      hidden={!Fillable.has('applications', 'description')}
                    />
                  </div>
                </div>
              </div>
          ...
          {/* Pestaña de imágenes */}
          {(Fillable.has('applications', 'image') || Fillable.has('applications', 'background_image')) && (
            <div className="col-12">
              <div className="card border-0 shadow-sm">
                <div className="card-header bg-light">
                  <h6 className="mb-0">
                    ...
                    {Fillable.has('applications', 'image') && (
                      <div className="col-md-6">
                        ...
                        <small className="text-muted">
                          ...
                          Formato cuadrado recomendado (ej: 512x512px)
                        </small>
                      </div>
                    )}
                    {Fillable.has('applications', 'background_image') && (
...
CreateReactScript((el, properties) => {
  createRoot(el).render(<BaseAdminto {...properties} title='Aplicaciones'>
