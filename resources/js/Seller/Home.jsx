...
const Home = ({
    ...
    if (nextRank) {
        ...
        const useGroup = nextRank.min_group_items > 0 && (nextRank.min_personal_items <= 0 || nextRank.requirement_logic === 'AND');
        
        currentVal = useGroup
            ? (isItems ? user.group_items : user.group_points)
            : (isItems ? user.total_items : user.total_points);
            
        targetVal = useGroup
            ? (isItems ? nextRank.min_group_items : nextRank.min_points)
            : (isItems ? nextRank.min_personal_items : nextRank.min_points);

        progress = targetVal > 0 ? Math.min(100, Math.max(0, (currentVal / targetVal) * 100)) : 0;
        ...
        // Etiqueta secundaria para indicar si es meta Grupal o Personal
        const isGroupMeta = useGroup;
    }
                ...
                >
                    ...
                    >
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                            ...
                            <small className="text-muted">
                                {nextRank.min_group_items > 0 && (nextRank.min_personal_items <= 0 || nextRank.requirement_logic === 'AND')
                                    ? "Meta Grupal"
                                    : "Meta Personal"}
                            </small>
                        </div>
